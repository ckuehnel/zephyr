...
#define SYS_LOG_DOMAIN "cfb"
#define SYS_LOG_LEVEL CONFIG_SYS_LOG_FRAMEBUFFER_LEVEL
...
int cfb_print(struct device *dev, char *str, u16_t x, u16_t y)
{
	...
	SYS_LOG_ERR("Unsupported framebuffer configuration");
	return -1;
}
...
int cfb_framebuffer_init(struct device *dev)
{
	...
	SYS_LOG_DBG("number of fonts %d", fb->numof_fonts);
	if (!fb->numof_fonts) {
		return -1;
	}
