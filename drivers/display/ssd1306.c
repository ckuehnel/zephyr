...
int ssd1306_write_ram(const struct device *dev, const u16_t x, const u16_t y,
		      ...
		      const u16_t p, const void *data)
{
	...
	for (size_t pidx = 0; pidx < SSD1306_PANEL_NUMOF_PAGES; pidx++) {
		if (ssd1306_write_page(dev, pidx, data, SSD1306_PANEL_WIDTH)) {
			return -1;
		}
		data = (u8_t *)data + SSD1306_PANEL_WIDTH;
	}
#endif
...
static int ssd1306_read_bitmap(const struct device *dev, const u16_t x,
...
static struct display_driver_api ssd1306_driver_api = {
	.display_on = ssd1306_resume,
	.display_off = ssd1306_suspend,
	.write_bitmap = ssd1306_write_ram,
	.read_bitmap = ssd1306_read_bitmap,
