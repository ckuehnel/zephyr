...
#define SYS_LOG_LEVEL CONFIG_SYS_LOG_DISPLAY_DRIVER_LEVEL
#define SYS_LOG_DOMAIN "ssd1673"
...
int ssd1673_write_ram(const struct device *dev, const u16_t x, const u16_t y,
...
static int ssd1673_read_bitmap(const struct device *dev, const u16_t x,
			const u16_t y, const u16_t w, const u16_t h,
			const u16_t p, void *data)
{
	SYS_LOG_ERR("not supported");
	return -ENOTSUP;
}
...
{
	SYS_LOG_WRN("not supported");
	return -ENOTSUP;
}

int ssd1673_set_contrast(const struct device *dev, u8_t contrast)
{
	struct ssd1673_data *driver = dev->driver_data;
...
static int ssd1673_panel_init(struct device *dev)
{
	struct ssd1673_data *driver = dev->driver_data;
	u8_t tmp[3];
	...
	tmp[0] = (SSD1673_RAM_YRES - 1); tmp[1] = 0;
...
static int ssd1673_init(struct device *dev)
{
	...
	if (driver->spi_dev == NULL) {
		SYS_LOG_ERR("Could not get SPI device for SSD1673");
		return -EIO;
	}
	...
	if (driver->reset == NULL) {
		SYS_LOG_ERR("Could not get GPIO port for SSD1673 reset");
		return -EIO;
	}
	...
	if (driver->dc == NULL) {
		SYS_LOG_ERR("Could not get GPIO port for SSD1673 DC signal");
		return -EIO;
	}
	...
	if (driver->busy == NULL) {
		SYS_LOG_ERR("Could not get GPIO port for SSD1673 busy signal");
		return -EIO;
	}
	...
	if (!driver->cs_ctrl.gpio_dev) {
		SYS_LOG_ERR("Unable to get SPI GPIO CS device");
		return -EIO;
	}
...
static struct display_driver_api ssd1673_driver_api = {
	.display_on = ssd1673_resume,
	.display_off = ssd1673_suspend,
	.write_bitmap = ssd1673_write_ram,
	.read_bitmap = ssd1673_read_bitmap,
