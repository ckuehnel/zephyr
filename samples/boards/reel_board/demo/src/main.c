...
static int test_exp_con(void)
{
	...
	/*
	gpio_pin_configure(dev_port0, EXPCON_TPIN8, GPIO_DIR_OUT);
	gpio_pin_configure(dev_port0, EXPCON_TPIN9, GPIO_DIR_IN);
	*/
	...
	for (int i = 0; i < 100; i++) {
		...
		/*
		gpio_pin_write(dev_port0, EXPCON_TPIN8, 1);
		gpio_pin_read(dev_port0, EXPCON_TPIN9, &tmp1);
		gpio_pin_write(dev_port0, EXPCON_TPIN8, 0);
		gpio_pin_read(dev_port0, EXPCON_TPIN9, &tmp2);
		if (tmp1 != 1 || tmp2 != 0) {
			printk("Failed test GPIO EXCON_TPIN8\n");
			return -1;
		}
		*/
